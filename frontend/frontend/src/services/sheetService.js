...
const sheetService = {
  ...
  // Generation team: Select an entry to work on
  selectEntry: async (sheetId, entryId) => {
    try {
      const response = await apiClient.post(`/sheets/${sheetId}/entries/${entryId}/select`);
      return response.data;
    } catch (error) {
      throw error.response?.data || { message: 'Failed to select entry' };
    }
  },
